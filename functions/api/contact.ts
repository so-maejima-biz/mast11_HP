...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (message.length > 5000) {
      return new Response(
        JSON.stringify({ success: false, error: 'お問い合わせ内容が長すぎます' }),
        { status: 400, headers }
      );
    }
    ...
    if (!isValidEmail(email)) {
      return new Response(
        JSON.stringify({ success: false, error: 'メールアドレスの形式が正しくありません' }),
        { status: 400, headers }
      );
    }
    ...
    if (!env.TO_EMAIL || !env.RESEND_API_KEY) {
      ...
      if (!env.RESEND_API_KEY) {
        ...
        console.log('[Contact] Form data:', { name, email: '[REDACTED]', company, message: message.slice(0, 100) + '...' });
        return new Response(
          JSON.stringify({ success: true, message: 'お問い合わせを受け付けました（開発モード）' }),
          { status: 200, headers }
        );
      }
      return new Response(
        JSON.stringify({ success: false, error: 'サーバー設定エラーが発生しました' }),
        { status: 500, headers }
      );
    }
...
このメールはシステムから自動送信されています。
    ...
    return new Response(
      JSON.stringify({ success: false, error: 'エラーが発生しました。時間をおいて再度お試しください。' }),
      { status: 500, headers }
    );
  }
};
